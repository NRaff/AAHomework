...
def craiglockhart_and_tollcross
  ...
  join routes b
  on (a.company = b.company and a.num = b.num)
  where a.stop_id = (select id from stops where name = 'Craiglockhart')
  and b.stop_id = (select id from stops where name = 'Tollcross')
  SQL
end
...
def start_at_craiglockhart
  ...
  JOIN
    routes b ON (a.company = b.company AND a.num = b.num)
  join stops s on (b.stop_id = s.id)
  WHERE
    a.stop_id = 53
  SQL
end
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    -- wtf
    select r.num, r.company, s.name, s.id
    from routes r
    join stops s on r.stop_id = s.id
    where s.name = 'Craiglockhart' or s.name = 'Sighthill'
